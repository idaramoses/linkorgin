...
function Page1({ togglePage }: Page1Props){
    ...
    try {
      ...
      const chatResults = await AuthService.searchGovtChat(searchQuery,'Legal','Immigration Lawyers');
  ...
  try {
    ...
    const chatResults = await AuthService.searchGovtChat(quicklinks,'Legal','Immigration Lawyers');
    setSearchResult(chatResults);
    setSearchQuery('');
    setLoading(false);
    setisSearched(true);
      ...
      <div className="flex flex-col self-start  pt-2 text-white rounded-lg bg-zinc-500 w-56">
        ...
        <div className="flex z-10 gap-4 items-center justify-center py-2 mt-1.5  text-base font-semibold whitespace-nowrap bg-red-500 rounded-b-md md:pr-5">
          ...
          <Img src="/images/img_settings.svg" alt="home_one" className="h-[23px] self-center" />
            ...
            <div className="my-auto text-white-A700 text-xs"> Legal & Immigration</div>
          </div>
          <img
            loading="lazy"
      ...
      <div className="mt-16 w-full md:mt-10 md:w-full">
        <div className="flex gap-2 md:flex-col max-md:gap-0">
          <div className="flex flex-col w-3/12 max-md:ml-0 md:w-full">
            <div onClick={() => handlequicklink('Unsure which legal path is best for you? I can explain the differences between Immigration Lawyers, Legal Aid Organizations','Immigration and Citizenship')} className="flex flex-col h-40  cursor-pointer items-center grow px-4 py-2 w-full bg-white rounded-xl border border-solid border-zinc-300 md:mt-4">
              
              <div className="mt-9 text-sm text-neutral-700">
              Unsure which legal path is best for you?.
              </div>
            </div>
          </div>
          <div className="flex flex-col ml-5 w-3/12 md:ml-0 md:w-full">
            <div onClick={() => handlequicklink('Are you a refugee seeking asylum or resettlement support','Social Services & Benefits')} className="grow px-4 py-2 h-40 flex  cursor-pointer items-center w-full  bg-white rounded-xl text-sm border border-solid border-zinc-300 text-neutral-700 max-md:mt-7">
            Are you a refugee seeking asylum or resettlement support
              <br />
            </div>
          </div>
          ...
          <div onClick={() => handlequicklink('Want to learn more about your legal rights and responsibilities in Canada?','Legal Aid')} className="flex flex-col ml-5 w-3/12 md:ml-0 md:w-full">
            <div className="grow px-4 py-2 h-40 flex  cursor-pointer  items-center w-full text-sm bg-white rounded-xl border border-solid border-zinc-300 text-neutral-700 max-md:mt-7">
            Want to learn more about your legal rights and responsibilities in Canada?
              <br />
            </div>
          </div>
        </div>
