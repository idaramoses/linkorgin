...
function Page1({ togglePage }: Page1Props){
    ...
    try {
      ...
      const chatResults = await AuthService.searchGovtChat(searchQuery,'Banking Services','Money Transfer Services');
  ...
  try {
    ...
    const chatResults = await AuthService.searchGovtChat(quicklinks,'Banking Services','Money Transfer Services');
    setSearchResult(chatResults);
    setSearchQuery('');
    setLoading(false);
    setisSearched(true);
  ...
  const handleResponse = (responseData: any): void => {
    ...
    // You can set the response data to state or perform any other actions based on the response
      ...
      <div className="flex flex-col self-start  pt-2 text-white rounded-lg bg-zinc-500 w-56">
        ...
        <div className="flex z-10 gap-4 items-center justify-center py-2 mt-1.5  text-base font-semibold whitespace-nowrap bg-red-500 rounded-b-md md:pr-5">
          ...
          <Img src= "/images/img_settings_white_a700.svg" alt="home_one" className="h-[23px] self-center" />
            ...
            <div className="my-auto text-white-A700 text-xs">Finance</div>
          </div>
          <img
            loading="lazy"
      ...
      <div className="mt-5 w-[50%]  text-sm text-black md:w-full">
       Have questions or need help understanding your financial options? Let's chat!
      </div>
      <div className="mt-16 w-full md:mt-10 md:w-full">
        <div className="flex gap-2 md:flex-col max-md:gap-0">
          <div className="flex flex-col w-3/12 max-md:ml-0 md:w-full">
            <div onClick={() => handlequicklink('Unsure which bank to choose or what investment options are best for you?','Immigration and Citizenship')} className="flex flex-col h-40  cursor-pointer items-center grow px-4 py-2 w-full bg-white rounded-xl border border-solid border-zinc-300 md:mt-4">
              
              <div className="mt-9 text-sm text-neutral-700">
               Unsure which bank to choose or what investment options are best for you?
              </div>
            </div>
          </div>
          <div className="flex flex-col ml-5 w-3/12 md:ml-0 md:w-full">
            <div onClick={() => handlequicklink('Interested in learning more about managing your money effectively?','Social Services & Benefits')} className="grow px-4 py-2 h-40 flex  cursor-pointer items-center w-full  bg-white rounded-xl text-sm border border-solid border-zinc-300 text-neutral-700 max-md:mt-7">
            Interested in learning more about managing your money effectively?
              <br />
            </div>
          </div>
          <div onClick={() => handlequicklink(' Are you a newcomer looking for small business loan options?','Employment Services')}  className="flex flex-col ml-5 w-3/12 md:ml-0 md:w-full">
            ...
            Are you a newcomer looking for small business loan options? 
            </div>
          </div>
