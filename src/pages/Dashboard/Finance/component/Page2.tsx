...
function Page2({ togglePage}: Page1Props){
  const [query, setQuery] = useState<string>('what investment options are best');
  ...
  const fetchGoogleSearchResults = async () => {
    ...
    try {
      ...
      const response = await axios.post<{ results: GoogleSearchResult[] }>(
        ...
        {"searchQuery": {"searchQuery":`${query} in canada` }},
        {
          headers: {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    fetchGoogleSearchResults();
      ...
      Hey there! Mon-Ami here, your friendly guide to navigating finances in Canada. Whether you need to find specific services or want personalized advice, I'm here to help you manage your money with confidence!
</div></div>}
        </div>
