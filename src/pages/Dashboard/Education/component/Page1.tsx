...
function Page1({ togglePage }: Page1Props){
    ...
    try {
      ...
      const chatResults = await AuthService.searchGovtChat(searchQuery,'education','english langauge school');
  ...
  try {
    ...
    const chatResults = await AuthService.searchGovtChat(quicklinks,'education','english langauge school');
    setSearchResult(chatResults);
    setSearchQuery('');
    setLoading(false);
    setisSearched(true);
  ...
  const handleResponse = (responseData: any): void => {
    ...
    // You can set the response data to state or perform any other actions based on the response
      ...
      <div className="flex flex-col self-start  pt-2 text-white rounded-lg bg-zinc-500 w-56">
        ...
        <div className="flex z-10 gap-4 items-center justify-center py-2 mt-1.5  text-base font-semibold whitespace-nowrap bg-red-500 rounded-b-md md:pr-5">
          ...
          <Img src="/images/img_settings.svg" alt="home_one" className="h-[23px] self-center" />
            ...
            <div className="my-auto text-white-A700 text-xs">Education</div>
          </div>
          <img
            loading="lazy"
      ...
      Hey there! Mon-Ami here, ready to guide you on your educational journey in Canada! Whether you're looking to brush up on some skills, learn a new language, or even pursue higher education, I can help you find the perfect program.
      </div>
      <div className="mt-16 w-full md:mt-10 md:w-full">
        <div className="flex gap-2 md:flex-col max-md:gap-0">
          <div className="flex flex-col w-3/12 max-md:ml-0 md:w-full">
            <div onClick={() => handlequicklink('Are you interested in Adult Education Programs to develop new skills or update existing ones?','Immigration and Citizenship')} className="flex flex-col h-40  cursor-pointer items-center grow px-4 py-2 w-full bg-white rounded-xl border border-solid border-zinc-300 md:mt-4">
              
              <div className="mt-9 text-sm text-neutral-700">
              Are you interested in Adult Education Programs to develop new skills or update existing ones?
              </div>
            </div>
          </div>
          <div className="flex flex-col ml-5 w-3/12 md:ml-0 md:w-full">
            <div onClick={() => handlequicklink('Want to improve your English or French communication? I can help you find the perfect English Language School or resources for learning French','Social Services & Benefits')} className="grow px-4 py-2 h-40 flex  cursor-pointer items-center w-full  bg-white rounded-xl text-sm border border-solid border-zinc-300 text-neutral-700 max-md:mt-7">
           I can help you find the perfect English Language School or resources for learning French.
              <br />
            </div>
          </div>
          <div onClick={() => handlequicklink('Do you dream of attending a college or university in Canada?','Employment Services')}  className="flex flex-col ml-5 w-3/12 md:ml-0 md:w-full">
            ...
            Do you dream of attending a college or university in Canada? 
            </div>
          </div>
